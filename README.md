...
* скачайте сборку: ```git clone git@github.com:evgenyzhernoklev/avon.git```;
* установите ```gulp``` глобально: ```yarn global add gulp-cli```;
* перейдите в скачанную папку со сборкой: ```cd avon```;
...
* SCSS-файлы находятся в папке ```src/styles```
	* стили блоков: ```src/styles/blocks```
	...
	* сторонние библиотеки (от других разработчиков): ```src/styles/vendor```
...
В данном коде мы определяем зависимости, затем Browserify собирает их в один файл. Также Browserify умеет создавать sourcemaps до компрессии, поэтому не смотря на конкатенацию, вы сможете отлаживать отдельные части пакета ровно так же, как вы и привыкли это делать с отдельными файлами.
